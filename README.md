...
- 🔑 **[BUGFIX] Carregamento Local de APIs** (`compilador.py`): Adicionado carregamento nativo e independente de dependências do arquivo `.env` para carregar as chaves `SERPER_API_KEY` e `SCRAPER_API_KEY` quando executado localmente.
- ⚙️ **[BUGFIX] Classificação de Nível no Serper** (`compilador.py`): Ajustada a estrutura de `if/elif/else` para priorizar a detecção de `"especializ"` ou `"lato"` no título e na URL do edital, impedindo que menus/links do rodapé de outros IFs categorizassem a página como Stricto Sensu (Mestrado/Aluno Especial) e causassem o seu descarte incorreto.
