...
- 🎨 **[VISUAL] Identidade YLuna85 LABs Unificada** (`index.html`): Design System migrado de **Light Mode branco** (`#FAFAFA`) para o **Dark Mode oficial** `#0D1117`. Paleta unificada com a marca — **Ciano Elétrico** `#00D4FF` (primário), **Roxo Neon** `#9B59FF` (secundário), gradiente de fundo neon tricolor e cores por nível de pós (Mestrado=Ciano, Doutorado=Roxo, Especial=Âmbar).
- ✅ **[BUGFIX] Lógica de Gratuidade Corrigida** (`compilador.py`): Corrigida classificação incorreta de UFBA e UNEB como `"Paga"`. Nova regra: em **instituições públicas federais e estaduais** (UFBA, UNEB, UEFS, UESC, UESB, IFBA, IF Baiano, etc.), o campo `gratuidade` é sempre `"Gratuita"` — a taxa de inscrição no processo seletivo **não** equivale ao custo do curso. Apenas editais de instituições privadas sem menção explícita de gratuidade são classificados como `"Paga"`.
