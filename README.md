...
- ✅ **[BUGFIX] Lógica de Gratuidade Corrigida** (`compilador.py`): Corrigida classificação incorreta de UFBA e UNEB como `"Paga"`. Nova regra: em **instituições públicas federais e estaduais** (UFBA, UNEB, UEFS, UESC, UESB, IFBA, IF Baiano, etc.), o campo `gratuidade` is sempre `"Gratuita"` — a taxa de inscrição no processo seletivo **não** equivale ao custo do curso. Apenas editais de instituições privadas sem menção explícita de gratuidade são classificados como `"Paga"`.
