...
- 🎨 **[VISUAL] Identidade YLuna85 LABs Unificada & Polimento Dark** (`index.html`): 
  - Design System migrado de **Light Mode branco** (`#FAFAFA`) para o **Dark Mode oficial** `#0D1117`. Paleta unificada com a marca — **Ciano Elétrico** `#00D4FF` (primário), **Roxo Neon** `#9B59FF` (secundário), gradiente de fundo neon tricolor e cores por nível de pós (Mestrado=Ciano, Doutorado=Roxo, Especial=Âmbar).
  - Remoção de todos os resíduos claros (fundo `#e2e8f0` da status bar e dashboard de busca, campos de input brancos com texto preto, botões de chip escuros com hover preto). Todos os elementos foram adaptados com transparências brancas (`rgba(255,255,255,0.04)`), bordas em neon ciano/roxo (`var(--border-glow)`) e textos claros de alto contraste.
  - Reestilização do ícone de logotipo (`🔬`) para um badge circular com gradiente neon translúcido e sombra com glow, além de borda inferior do cabeçalho com degradê gradativo.
  - Ajuste de badges de níveis e status para estilo **glassmorphism** translúcido em vez de blocos opacos de cores sólidas.
- 🌐 **[ESCOPO] Especializações Nacionais & Stricto Sensu Local**: Configurada a restrição de localidade para descartar editais de Mestrado e Doutorado de fora da Bahia, mantendo o escopo 100% nacional para Especializações (Lato Sensu), com foco em EaD e EPT.
- 🔑 **[BUGFIX] Carregamento Local de APIs** (`compilador.py`): Adicionado carregamento nativo do arquivo `.env` para carregar as chaves `SERPER_API_KEY` e `SCRAPER_API_KEY` quando executado localmente.
- ⚙️ **[BUGFIX] Classificação de Nível no Serper** (`compilador.py`): Ajustada a estrutura de `if/elif/else` para priorizar a detecção de `"especializ"` ou `"lato"` no título e na URL do edital, impedindo que menus/links do rodapé de outros IFs categorizassem a página como Stricto Sensu.
- 📅 **[BUGFIX] Heurística Inteligente de Datas** (`compilador.py`): Implementado um algoritmo de pontuação de datas e expressões regulares de intervalos (`DD/MM/AAAA a DD/MM/AAAA`) que analisa o contexto ao redor da data (como proximidade com as palavras "inscrição", "prazo", "término" vs. "aula", "curso"). Isso corrigiu o bug que confundia a data de início das aulas do IFSP com o encerramento das inscrições.
- 🔄 **[MELHORIA] Retificação de Prazos no Histórico** (`compilador.py`): Alterado o gatilho de atualização de prazos do histórico de maior (`>`) para diferente (`!=`), assegurando que revisões de datas anteriores ou retificações de editais sejam gravadas nas bases locais.
- ⚡ **[MELHORIA] Camada de Cache Local** (`compilador.py`): Adicionado mecanismo de cache MD5 na função `http_request` que salva respostas bem-sucedidas em arquivos JSON locais na pasta `.cache/` com TTL de 12 horas. Isso previne o desperdício de créditos de API Serper/ScraperAPI em execuções repetidas durante testes.
- 🔍 **[MELHORIA] Filtro de EPT Nacional** (`compilador.py`): Inclusão de queries de busca específicas para especializações em Gestão na EPT e Docência na EPT no Serper, permitindo capturar novos editais nacionais de forma automatizada.
