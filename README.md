...
- ✅ **[BUGFIX] Lógica de Gratuidade Corrigida** (`compilador.py`): Corrigida classificação incorreta de UFBA e UNEB como `"Paga"`. Nova regra: em **instituições públicas federais e estaduais** (UFBA, UNEB, UEFS, UESC, UESB, IFBA, IF Baiano, etc.), o campo `gratuidade` é sempre `"Gratuita"`. Incluída a verificação de pertencimento no título do edital (`titulo_lower`) e criados scripts utilitários (`corrigir_gratuidade_existente.py` e `recompilar_dados_pos_bahia.py`) para corrigir retroativamente as bases históricas (`DATA/` e `ultimos-editais.json`) convertendo 102 ocorrências erroneamente marcadas de Paga para Gratuita.
