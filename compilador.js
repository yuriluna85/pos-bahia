...
function salvarHistoricoEdital(tema, editais, dataEspecifica = null) {
  ...
  editais.forEach(e => {
    const index = historicoDia.findIndex(h => h.url === e.url && normalizarChaveDedup(h.titulo) === normalizarChaveDedup(e.titulo));
    if (index !== -1) {
...
function gerarMetricas() {
  ...
  todosEditais.forEach(e => {
    const chave = `${normalizarChaveDedup(e.titulo)}-${e.url}`;
...
function corrigirUtf8Corrompido(texto) {
  if (!texto) return '';
  try {
    // Detect Latin-1-as-UTF8 corruption using explicit code points
    const isCorrupt = (
      texto.includes('\u00c3\u00a7') ||
      texto.includes('\u00c3\u00a3') ||
      texto.includes('\u00c3\u00a1') ||
...
// Normalizes a title: strips NBSP, replacement chars and extra spaces
function normalizarTitulo(texto) {
  if (!texto) return '';
  return texto.replace(/\xa0/g, ' ').replace(/\u00a0/g, ' ').replace(/\ufffd/g, '').replace(/\s+/g, ' ').trim();
}

// Full dedup key: fixes encoding AND normalizes whitespace.
// Ensures 'Gest\u00c3\u00a3o' and 'Gest\u00e3o' are the same edital.
function normalizarChaveDedup(titulo) {
  return normalizarTitulo(corrigirUtf8Corrompido(titulo));
}
