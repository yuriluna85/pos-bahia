...
async function buscarNovosEditais() {
    ...
    try {
      ...
      if (searchRes.statusCode === 200) {
        ...
        for (const item of items.slice(0, 5)) { // Limita aos 5 primeiros resultados por query
          ...
          if (ehUrlGenerica(url)) {
            console.log(`[Filtro] Ignorando URL genérica de landing page: ${url}`);
            continue;
          }
          ...
          try {
            ...
            if (scrapeRes.statusCode === 200) {
              ...
              // Evita falsos positivos de páginas antigas (ex: 2025) ignorando se não achar datas de 2026
              if (dates.length < 2) {
                console.log(`[Scraper] Ignorando ${url} - Datas de inscrição de 2026 insuficientes.`);
                continue;
              }

              dates.sort((a, b) => a - b);
              let inscInicio = dates[0].toISOString();
              let inscFim = dates[dates.length - 1].toISOString();

              // Filtra editais encerrados antes de Junho/2026 (início do histórico a partir de agora)
              const limiteHistorico = new Date('2026-06-01T00:00:00.000Z');
              if (new Date(inscFim) < limiteHistorico) {
                console.log(`[Scraper] Ignorando ${url} - Inscrições encerradas antes de Junho/2026.`);
                continue;
              }
  ...
  // Sempre tenta raspar os portais SIGAA (UFBA e UNILAB) diretamente para ter editais reais garantidos
  const sigaaPortais = [
...
async function buscarNovosEditaisSimulados() {
  ...
  // Além dos simulados, tenta raspar os editais reais do SIGAA (UFBA e UNILAB)
  const sigaaPortaisSimulados = [
    { sigla: 'UFBA', url: 'https://sigaa.ufba.br/sigaa/public/processo_seletivo/lista.jsf?aba=p-processo&nivel=S' },
    { sigla: 'UNILAB', url: 'https://sigaa.unilab.edu.br/sigaa/public/processo_seletivo/lista.jsf?aba=p-processo&nivel=S' }
  ];
  for (const portal of sigaaPortaisSimulados) {
    try {
      const portalEditais = await rasparSigaaPortalDirect(portal.sigla, portal.url);
      console.log(`[SIGAA ${portal.sigla}] Encontrados ${portalEditais.length} editais reais em modo simulação.`);
      portalEditais.forEach(ed => {
        let pasta = "mestrado";
        if (ed.nivel === "Aluno Especial") {
          pasta = "aluno-especial";
      ...
      console.error(`Erro ao raspar SIGAA ${portal.sigla} no modo simulação:`, err.message);
    }
  }
